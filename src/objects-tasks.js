...
function shallowCopy(obj) {
  const newArray = {};
  return Object.assign(newArray, obj);
}
...
function mergeObjects(objects) {
  ...
  Object.values(newObj).reduce((acc, obj) => {
...
function makeWord(lettersObject) {
  const allValues = Object.values(lettersObject).flat().sort();

  let word = '';

  allValues.forEach((value) => {
    const letter = Object.keys(lettersObject).find((key) =>
...
function sellTickets(queue) {
  const res = queue.reduce((changeTicket, payTicket) => {
    if (payTicket === 25) {
      return changeTicket + payTicket;
    }
    return changeTicket - payTicket;
  }, 0);
  return res >= 0;
